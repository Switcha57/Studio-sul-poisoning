#!/usr/bin/env python3
"""
Testing script for fine-tuned DNN malware classifier.
Loads a fine-tuned model and evaluates it on test dataset.
"""
...
def test_fine_tuned_model(config):
    """Main testing function for fine-tuned model."""
    dataset_config = config['DATASET']
    fine_tune_config = config['FINE_TUNING']
    testing_config = config['TESTING']
    
    # Load fine-tuned model
    model_path = fine_tune_config.get('fine_tuned_model_save_path', 'models/dnn_finetuned.h5')
...
def main():
    ...
    print("="*50)
    print("DNN Malware Classifier - Fine-Tuned Model Testing")
    print("="*50 + "\n")
    
    try:
        config = load_config(config_path)
        model, results = test_fine_tuned_model(config)
