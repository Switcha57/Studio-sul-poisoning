#!/usr/bin/env python3
"""
Fine-tuning script for DNN malware classifier.
Loads a pre-trained model from config.ini and fine-tunes it on a new dataset.
"""
...
def load_base_model(config):
    """Load and return the base model for fine-tuning."""
    fine_tune_config = config['FINE_TUNING']
    
    base_model_path = fine_tune_config.get('base_model_path')
    
    if not base_model_path:
        raise ValueError("base_model_path must be specified in [FINE_TUNING] section")
...
def freeze_layers(model, freeze_indices):
    """Freeze specified layers in the model."""
    if freeze_indices is None or freeze_indices == 'none':
        print("No layers frozen - all layers will be trainable")
        return model
    
    # Parse freeze indices
    if isinstance(freeze_indices, str):
        if freeze_indices.lower() == 'none':
            print("No layers frozen - all layers will be trainable")
            return model
        freeze_indices = [int(idx.strip()) for idx in freeze_indices.split(',')]
    
    print(f"\nFreezing layers at indices: {freeze_indices}")
...
def compile_for_fine_tuning(model, config):
    """Compile the model with fine-tuning specific settings."""
    fine_tune_config = config['FINE_TUNING']
    
    learning_rate = float(fine_tune_config.get('fine_tune_learning_rate', 0.0001))
...
def fine_tune_model(config):
    ...
    freeze_indices = fine_tune_config.get('freeze_layers', 'none')
    model = freeze_layers(model, freeze_indices)
    
    # Compile for fine-tuning
    model = compile_for_fine_tuning(model, config)
    
    print("\nModel architecture (after freezing):")
    model.summary()
    ...
    epochs = int(fine_tune_config.get('fine_tune_epochs', 500))
    batch_size = int(training_config.get('batch_size', 256))
    patience = int(fine_tune_config.get('fine_tune_early_stopping_patience', 15))
    ...
    model_save_path = fine_tune_config.get('fine_tuned_model_save_path', 'models/dnn_finetuned.h5')
...
def main():
    ...
    print("="*50)
    print("DNN Malware Classifier - Fine-Tuning Script")
    print("="*50 + "\n")
    
    try:
        config = load_config(config_path)
        model, history = fine_tune_model(config)
